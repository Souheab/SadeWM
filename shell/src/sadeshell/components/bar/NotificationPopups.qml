    ...
    component NotificationToast: Rectangle {
        ...
        NumberAnimation {
            ...
            onFinished: {
                ...
                root.inputRegionChanged()
                NotificationService.toastShown(toast.notif.id)
            }
        }
        ...
        Connections {
            ...
            function onToastExpired(ident) {
                if (toast.notif.id === ident && !slideOutAnim.running)
                    slideOutAnim.start()
            }
        }
        ...
        MouseArea {
            anchors.fill: parent
            hoverEnabled: true
            onEntered: NotificationService.setPaused(toast.notif.id, true)
            onExited: {
                if (!slideOutAnim.running)
                    NotificationService.setPaused(toast.notif.id, false)
            }
            onClicked: {
                if (!slideOutAnim.running)
                    slideOutAnim.start()
