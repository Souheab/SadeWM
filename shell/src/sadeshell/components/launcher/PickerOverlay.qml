...
Window {
    ...
    function reportRequestedWindows() {
        var ids = []
        if (opened) {
            var first = Math.max(0, Math.floor(gridView.contentY / cellHeight) * columns)
            var end = Math.min(windowCount, first + (Math.ceil(gridView.height / cellHeight) + 1) * columns)
            for (var row = first; row < end; row++)
                ids.push(windowModel.get(row).winId)
        }
        WindowPickerService.setRequestedWindows(minimizedOnly ? "minimized" : "normal", ids, opened)
    }
    onOpenedChanged: Qt.callLater(reportRequestedWindows)
    onWindowCountChanged: Qt.callLater(reportRequestedWindows)
    ...
    Connections {
        ...
        function onWindowsChanged() {
            if (!picker.minimizedOnly && picker.opened)
                Qt.callLater(picker.reportRequestedWindows)
    ...
    Rectangle {
        ...
        Item {
            ...
            GridView {
                ...
                reuseItems: true
                onContentYChanged: Qt.callLater(picker.reportRequestedWindows)
                onHeightChanged: Qt.callLater(picker.reportRequestedWindows)
