...
class _WatcherInterface(ServiceInterface if HAS_DBUS else object):
    ...
    if HAS_DBUS:
        ...
        @method()
        def RegisterStatusNotifierItem(self, service: "s"):  # noqa: F821, F722 -- D-Bus wire signatures, not Python types
            self._svc._on_item_registered(service)

        @method()
        def RegisterStatusNotifierHost(self, service: "s"):  # noqa: F821, F722 -- D-Bus wire signatures, not Python types
            self._svc._host_registered = True
            self.StatusNotifierHostRegistered()

        @dbus_property(access=PropertyAccess.READ)
        def RegisteredStatusNotifierItems(self) -> "as":  # noqa: F821, F722 -- D-Bus wire signatures, not Python types
            return self._svc._registered_service_ids()

        @dbus_property(access=PropertyAccess.READ)
        def IsStatusNotifierHostRegistered(self) -> "b":  # noqa: F821, F722 -- D-Bus wire signatures, not Python types
            return True

        @dbus_property(access=PropertyAccess.READ)
        def ProtocolVersion(self) -> "i":  # noqa: F821, F722 -- D-Bus wire signatures, not Python types
            return 0

        @dbus_signal()
        def StatusNotifierItemRegistered(self, service: "s"):  # noqa: F821, F722 -- D-Bus wire signatures, not Python types
            pass

        @dbus_signal()
        def StatusNotifierItemUnregistered(self, service: "s"):  # noqa: F821, F722 -- D-Bus wire signatures, not Python types
            pass
