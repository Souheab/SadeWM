...
from sadeshell.services.shared.window_picker_service import (  # noqa: E402
    WindowPickerService,
    WindowListModel,
    _WindowAssets,
...
class TestWindowPickerService(unittest.TestCase):
    def test_icon_capture_failure_uses_shared_theme_lookup(self):
        from sadeshell.services.shared import window_picker_service as picker
        capture = mock.Mock()
        capture.icon.side_effect = RuntimeError("window vanished")
        with mock.patch.object(picker, "_capture_connection", return_value=capture), mock.patch.object(
            picker, "_icon_path_from_class", return_value="/theme/fallback.svg"
        ):
            self.assertEqual(_net_wm_icon_file_uri(1, "Gone"), "file:///theme/fallback.svg")

    def test_capture_reuses_connection_after_window_vanishes(self):
        from sadeshell.services.shared import window_picker_service as picker
        capture = mock.Mock()
        capture.capture.side_effect = RuntimeError("window vanished")
        with mock.patch.object(picker._CAPTURE_LOCAL, "connection", capture, create=True):
            self.assertEqual(_capture_thumbnail_file_uri(1), "")
            self.assertEqual(_capture_thumbnail_file_uri(2), "")
    ...
    def test_cached_assets_are_reused_in_metadata(self):
        exists = mock.patch("sadeshell.services.shared.window_picker_service.os.path.isfile", return_value=True)
        exists.start()
    ...
    def test_normal_refresh_excludes_minimized_windows(self):
        ...
        with mock.patch(
            "sadeshell.services.shared.window_picker_service._sadewm_request",
