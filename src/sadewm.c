...
enum { NetSupported, NetWMName, NetWMState, NetWMCheck,
       ...
       NetWMStateAbove, NetWMStateStaysOnTop,
...
clientmessage(XEvent *e)
{
	...
	if (cme->message_type == netatom[NetWMState]) {
		...
		if (cme->data.l[1] == netatom[NetWMStateAbove]
		|| cme->data.l[2] == netatom[NetWMStateAbove]
		|| cme->data.l[1] == netatom[NetWMStateStaysOnTop]
		|| cme->data.l[2] == netatom[NetWMStateStaysOnTop])
			setabove(c, (cme->data.l[0] == 1 /* _NET_WM_STATE_ADD    */
				|| (cme->data.l[0] == 2 /* _NET_WM_STATE_TOGGLE */ && !c->isabove)));
...
focus(Client *c)
{
	...
	if (c) {
		...
		attachstack(c);
		grabbuttons(c, 1);
		XSetWindowBorder(dpy, c->win, border_sel);
		setfocus(c);
	} else {
...
focusstack(const Arg *arg)
{
	...
	if (!selmon->sel || (selmon->sel->isfullscreen && lockfullscreen))
		return;
	if (arg->i > 0) {
		for (c = selmon->sel->next; c && !(ISVISIBLE(c) && !c->isdock); c = c->next);
		if (!c)
			for (c = selmon->clients; c && !(ISVISIBLE(c) && !c->isdock); c = c->next);
	} else {
...
manage(Window w, XWindowAttributes *wa)
{
  ...
  c->maximized = false;
	...
	XSetWindowBorder(dpy, w, border_norm);
...
propertynotify(XEvent *e)
{
	...
	if ((ev->window == root) && (ev->atom == XA_WM_NAME)) {
		/* status text update ignored: no built-in bar */
...
restack(Monitor *m)
{
	...
	if (!m->sel)
		return;
	if (m->sel->isfloating || m->sel->isabove || !m->lt->arrange)
		XRaiseWindow(dpy, m->sel->win);
	if (m->lt->arrange) {
		wc.stack_mode = Below;
		wc.sibling = root;
		for (c = m->stack; c; c = c->snext)
			if (!c->isfloating && !c->isabove && ISVISIBLE(c)) {
	...
	for (c = m->stack; c; c = c->snext)
		if (ISVISIBLE(c) && (c->isfloating || c->isabove))
			XRaiseWindow(dpy, c->win);
	XSync(dpy, False);
	while (XCheckMaskEvent(dpy, EnterWindowMask, &ev));
}
...
setabove(Client *c, int above)
{
	if (above && !c->isabove) {
		XChangeProperty(dpy, c->win, netatom[NetWMState], XA_ATOM, 32,
			PropModeReplace, (unsigned char*)&netatom[NetWMStateAbove], 1);
		c->isabove = 1;
		XRaiseWindow(dpy, c->win);
...
setup(void)
{
	...
	/* init border colors */
	{
		...
		XftColorAllocName(dpy, vis, cmap, col_border_norm, &xc);
		border_norm = xc.pixel;
		XftColorAllocName(dpy, vis, cmap, col_border_sel, &xc);
		border_sel  = xc.pixel;
		XftColorFree(dpy, vis, cmap, &xc); /* only free the last one; pixels remain valid */
	}
	/* init bars */
	updatebars();
	updatestatus();
...
unfocus(Client *c, int setfocus)
{
	if (!c)
		return;
	grabbuttons(c, 0);
	XSetWindowBorder(dpy, c->win, border_norm);
	if (setfocus) {
		XSetInputFocus(dpy, root, RevertToPointerRoot, CurrentTime);
		XDeleteProperty(dpy, root, netatom[NetActiveWindow]);
	}
...
updatestatus(void)
{
	/* no built-in bar; status text is unused */
}
...
updatewindowtype(Client *c)
{
	...
	if (state == netatom[NetWMFullscreen])
		setfullscreen(c, 1);
	if (state == netatom[NetWMStateAbove] || state == netatom[NetWMStateStaysOnTop])
		setabove(c, 1);
